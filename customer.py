...
class Customer:
    ...
    def requestCertificate (self):
        ...
        brokerSocket_ssl.send (json.dumps ({'Request': 'Certificate', 'Data': data}).encode('utf-8'))
    ...
    def serve (self):
        while True:
            ...
            print ('Data: ' + data)
            if data['Request'] == 'Key':
                conn.send (json.dumps({'Response' : 'OK', 'Data' : self.publicKey.exportKey(), 'Signature' : ''}).encode('utf-8'))
            else:
                conn.send(json.dumps({'Response': 'Error', 'Data': 'Unknown Request', 'Signature' : ''}).encode('utf-8'))
            conn.close()
